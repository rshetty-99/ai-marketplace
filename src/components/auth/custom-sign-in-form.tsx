...
export function CustomSignInForm({ className }: CustomSignInFormProps) {
  ...
  // Auto-fill email if there's a pending sign-in from signup
  useEffect(() => {
    const pendingEmail = sessionStorage.getItem('pendingSignInEmail');
    if (pendingEmail) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        // Handle redirect - if there's a redirect URL, use it, otherwise check onboarding
        if (redirectUrl) {
          window.location.href = redirectUrl;
        } else {
          const hasCompletedOnboarding = localStorage.getItem(`onboarding-${result.createdUserId}`);
